...
def paired_paths_from_folder(folders, keys, filename_tmpl):
    ...
    assert len(keys) == 4, (
        'The len of keys should be 2 with [input_key, gt_key]. '
        f'But got {len(keys)}')
    input_folder, gt_folder, depth_folder, focal_folder = folders
    input_key, gt_key, depth_key, focal_key = keys

    input_paths = sorted(list(scandir(input_folder)))
    depth_paths = sorted(list(scandir(depth_folder)))
    focal_paths = sorted(list(scandir(focal_folder)))
    ...
    for idx in range(len(gt_paths)):
        ...
        focal_path = osp.join(focal_folder, focal_path)
        assert input_name in input_paths, (f'{input_name} is not in '
                                           f'{input_key}_paths.')
        gt_path = osp.join(gt_folder, gt_path)
        paths.append(
            dict([(f'{input_key}_path', input_path),
                  (f'{depth_key}_path', depth_path),
                    (f'{focal_key}_path', focal_path),
                  (f'{gt_key}_path', gt_path)]))
    return paths
