...
class Dataset_PairedImage(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.focal_folder = opt['dataroot_focal']
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            self.io_backend_opt['db_paths'] = [self.lq_folder, self.gt_folder, self.depth_folder, self.focal_folder]
            self.io_backend_opt['client_keys'] = ['lq', 'gt', 'depth', 'focal']
            self.paths = paired_paths_from_lmdb(
                [self.lq_folder, self.gt_folder, self.depth_folder, self.focal_folder], ['lq', 'gt', 'depth', 'focal'])
    ...
    def __getitem__(self, index):
        ...
        if self.opt['multi_lens']:
            # F = random.randint(0, 2)
            F = 0
            if F == 0:
                lens = torch.tensor(1.8)
                gt_path = gt_path.replace(self.opt['folder_name'], self.opt['F1.8_folder'])
            elif F == 1:
                lens = torch.tensor(2.8)
                gt_path = gt_path.replace(self.opt['folder_name'], self.opt['F2.8_folder'])
            elif F == 2:
                lens = torch.tensor(8.0)
                gt_path = gt_path.replace(self.opt['folder_name'], self.opt['F8_folder'])
        ...
        # except:
        #     raise Exception("depth path {} not working".format(depth_path))

        focal_path = self.paths[index]['focal_path']
        img_focal = cv2.imread(focal_path, cv2.IMREAD_GRAYSCALE)
        ...
        if self.opt['phase'] == 'train':
            ...
            img_depth, img_focal = padding(img_depth, img_focal, gt_size)
            img_focal = np.expand_dims(img_focal, axis=2)
            img_depth = np.expand_dims(img_depth, axis=2)

            # random crop
            img_gt, img_lq, img_depth, img_focal = paired_random_crop(img_gt, img_lq, img_depth, img_focal, gt_size, scale,
                                                gt_path)

            # flip, rotation augmentations
            if self.geometric_augs:
                img_gt, img_lq = random_augmentation(img_gt, img_lq)
                img_depth, img_focal = random_augmentation(img_depth, img_focal)
        else:
            img_focal = np.expand_dims(img_focal, axis=2)
            img_depth = np.expand_dims(img_depth, axis=2)
        ...
        img_gt, img_lq = img2tensor([img_gt, img_lq],
                                    bgr2rgb=True,
                                    float32=True)
        img_focal = torch.from_numpy(img_focal.transpose(2, 0, 1)).float()/255.0
        img_depth = torch.from_numpy(img_depth.transpose(2, 0, 1)).float()/255.0
        # normalize
        if self.mean is not None or self.std is not None:
            normalize(img_lq, self.mean, self.std, inplace=True)
            normalize(img_gt, self.mean, self.std, inplace=True)
            normalize(img_depth, self.mean, self.std, inplace=True)
            # normalize(img_focal, self.mean, self.std, inplace=True)
        if self.opt['multi_lens']:
            return {
                'lq': img_lq,
                'gt': img_gt,
